...
export function storeVerifiedSession(username) {
  try {
    ...
    if (!verifiedUsernames.includes(trimmedUsername)) {
      verifiedUsernames.push(trimmedUsername);
    ...
    storePreviousUsername(trimmedUsername);
...
export function isVerifiedInSession(username) {
  try {
    ...
    const verifiedUsernames = JSON.parse(sessionStorage.getItem('verifiedUsernames') || '[]');
    return verifiedUsernames.includes(trimmedUsername);
  } catch (error) {
